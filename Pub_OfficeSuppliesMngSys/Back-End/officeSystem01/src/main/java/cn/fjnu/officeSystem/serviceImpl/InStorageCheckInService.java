...
public class InStorageCheckInService implements cn.fjnu.officeSystem.service.InStorageCheckInService{
	...
	public String getInstorageHome(String json){
		...
		if (inStorageTypeId.equals("1")) {//采购入库
			...
			count=itemProcurementDao.selectUnInStorageItemProcurementCount(new ItemProcurement());
			if (count>0) {
				PageUtil<ItemProcurementExtend> page=new PageUtil<>(pageIndex, count, pageCount);
				List<ItemProcurementExtend> resultList=itemProcurementDao.selectUnInStorageItemProcurement(null,null,null,null,0,page.getDataStart(),page.getDataEnd());
	...
	public String getInstorageSumary(String json){
		...
		String agentId=jsonObject.optString("agentId");
		String operaterId=jsonObject.optString("operaterId");
		String itemProcurementId=jsonObject.optString("itemProcurementId");
		int count=0;//统计总记录条数
		count=inStorageCheckInDao.selectInStorageCheckInsByMapCount(supplierId, agentId, operaterId, itemProcurementId);
		Map<String, Object> resultMap=new HashMap<>();
		String result=null;
		if (count>0) {
			PageUtil<ItemProcurementExtend> page=new PageUtil<>(pageIndex, count, pageCount);
