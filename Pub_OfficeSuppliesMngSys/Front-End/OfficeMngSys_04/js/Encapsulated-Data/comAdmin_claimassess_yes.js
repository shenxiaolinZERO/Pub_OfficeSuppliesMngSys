...
function loadDate(pn){
    ...
    type:"post",
    dataType:"json",
    url:"http://localhost:8080/officeSystem/RecipientApply/getAuditRecipientApplies.do",
    ...
    success:function(data){
        ...
        if(1){
            ...
            for(var i=0;i<data.resultList.length;i++){
                ...
                html1 +='<td>'+array.recipientNum+'</td>';
                html1 +='<td>'+array.recipienterName+'</td>';
                html1 +='<td>'+array.applyTime+'</td>';
                //审核状态
                html1 +='<td>'+array.state+'</td>';
                html1 +='<td>'+array.auditTime+'</td>';
                //领用状态
                html1 +='<td>'+array.recipientState+'</td>';

                html1 +='<td>'+array.isValid+'</td>';
                html1 +='<td><button id="'+array.id+'" onclick="delete(this)">删除</button></td>';
                html1 +='</tr>';
            }
            $("tbody").append(html1);
...
function add(element){
    ...
    $.ajax({
        type:"post",
        url:"http://localhost:8080/officeSystem/OverTimeBorrowApply/getBorrowApplyWarningPassHome.do",
...
function searchById(){
    ...
    $.ajax({
        type:"post",
        url:"http://localhost:8080/officeSystem/OverTimeBorrowApply/getBorrowedXXAppliesByItemId.do",
        data:JSON.stringify(x),
        dataType:"json",
        header:{
            "Content-Type":"application/json",
