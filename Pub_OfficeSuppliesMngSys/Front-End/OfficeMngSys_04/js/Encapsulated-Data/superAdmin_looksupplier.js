/**
 * Created by user10 on 2016/6/23.
 */
...
function loadDate(pn){
    ...
    $.ajax({
        type:"post",
        dataType:"json",
        url:"http://localhost:8080/officeSystem/SupplierManager/getAllSupplier.do",
        ...
        success:function(data){
            ...
            if(1){
                ...
                for(var i=0;i<data.resultList.length;i++){
                    ...
                    html1 +='<td>'+array.id+'</td>';
                    html1 +='<td>'+array.fullName+'</td>';
                    html1 +='<td>'+array.shortName+'</td>';
                    html1 +='<td>'+array.supplierTypeName+'</td>';
                    html1 +='<td>'+array.linkman+'</td>';
                    html1 +='<td>'+array.cellPhoneNumber+'</td>';
                    html1 +='<td>'+array.phone+'</td>';
                    html1 +='<td>'+array.fax+'</td>';
                    html1 +='<td>'+array.postcode+'</td>';
                    html1 +='<td>'+array.address+'</td>';
                    html1 +='<td>'+array.remark+'</td>';
                    html1 +='<td><button id="'+array.id+'" onclick="add(this)">编辑</button><button id="'+array.id+'" onclick="add(this)">设为无效</button></td>';
                    html1 +='</tr>';
                }
                $("tbody").append(html1);
...
function add(element){
    ...
    $.ajax({
        type:"post",
        url:"http://localhost:8080/officeSystem/InstorageCheckIn/returnInStorage.do",
