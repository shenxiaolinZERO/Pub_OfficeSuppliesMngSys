...
function loadDate(pn){
    ...
    $.ajax({
        type:"post",
        dataType:"json",
        url:"http://localhost:8080/officeSystem/ItemProcurement/getItemProcurementApplyHome.do",
        ...
        success:function(data){
            ...
            if(1){
                ...
                for(var i=0;i<data.resultList.length;i++){
                    ...
                    html1 +='<td>'+array.measureUnitId+'</td>';
                    html1 +='<td>'+array.num+'</td>';
                    html1 +='<td>'+array.staffName+'</td>';
                    html1 +='<td>'+array.applyTime+'</td>';
                    html1 +='<td><button id="'+array.id+'" onclick="confirmPre(this)">确定预采购</button>&nbsp;<button id="'+array.id+'" onclick="notPass(this)">不通过</button></td>';
                    html1 +='</tr>';
                }
                $("tbody").append(html1);
...
//确定预采购
  function confirmPre(element){
        ...
        $.ajax({
            type:"post",
            url:"http://localhost:8080/officeSystem/ItemProcurement/confirmPreItemProcurement.do",
            data:JSON.stringify(x),
            dataType:"json",
            header:{
...
//不通过
    function notPass(element){
        ...
        $.ajax({
            type:"post",
            url:"http://localhost:8080/officeSystem/ItemProcurement/notPassItemProcurementApply.do",
            data:JSON.stringify(x),
            dataType:"json",
            header:{
                "Content-Type":"application/json",
